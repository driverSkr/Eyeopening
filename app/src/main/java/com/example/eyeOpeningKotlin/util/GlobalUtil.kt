...
object GlobalUtil {
     ...
     * @return 当前应用程序的包名。
     */
    val appPackage: String get() = EyeopeningApplication.context.packageName

    /**
     * 获取当前应用程序的名称。
     * @return 当前应用程序的名称。
     */
    val appName: String get() = EyeopeningApplication.context.resources.getString(EyeopeningApplication.context.applicationInfo.labelRes)

    /**
     * 获取当前应用程序的版本名。
     * @return 当前应用程序的版本名。
     */
    val appVersionName: String get() = EyeopeningApplication.context.packageManager.getPackageInfo(appPackage,0).versionName
    ...
    val appVersionCode: Long get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P){
        EyeopeningApplication.context.packageManager.getPackageInfo(appPackage,0).longVersionCode
    } else {
        EyeopeningApplication.context.packageManager.getPackageInfo(appPackage,0).versionCode.toLong()
    }
     ...
     * @return 开眼当前应用程序的版本名。
     */
    val eyeopeningVersionName: String get() = "6.3.1"

    /**
     * 获取开眼应用程序的版本号。
     * @return 开眼当前应用程序的版本号。
     */
    val eyeopeningVersionCode: Long get() = 6030012
    ...
    fun getDeviceSerial(): String{
        if (deviceSerial == null){
            ...
            if ("google" != appChannel || "samsung" != appChannel){
                try {
                    deviceId = Settings.Secure.getString(EyeopeningApplication.context.contentResolver,Settings.Secure.ANDROID_ID)
                } catch (e: Exception){
     ...
     */
    fun getString(resId: Int): String = EyeopeningApplication.context.resources.getString(resId)
     ...
     * @return 字符串资源id对应的字符串内容。
     */
    fun getDimension(resId: Int): Int = EyeopeningApplication.context.resources.getDimensionPixelOffset(resId)
     ...
     * @return 指定资源名的资源id。
     */
    fun getResourceId(name: String,type: String): Int = EyeopeningApplication.context.resources.getIdentifier(name,type, appPackage)
    ...
    fun getApplicationMetaData(key: String): String?{
        ...
        try {
            applicationInfo = EyeopeningApplication.context.packageManager.getApplicationInfo(
    ...
    fun isInstalled(packageName: String): Boolean{
        val packageInfo: PackageInfo? = try {
            EyeopeningApplication.context.packageManager.getPackageInfo(packageName,0)
        } catch (e: PackageManager.NameNotFoundException){
            null
        }
    ...
    fun getAppIcon(): Drawable{
        val packageManager = EyeopeningApplication.context.packageManager
